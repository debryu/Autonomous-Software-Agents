...
function evaluatePath(path){
    ...
    for(let i = 0; i < path.length - 1; i++){
        ...
        time = Sensing.travelTime(cellFromUniqueID(path[i].id),cellFromUniqueID(path[i+1].id));
    ...
    for (let i = 0; i < graph.length; i++) {
        ...
        for(let j = 0; j < ant.path.length; j++){
            ...
            if(graph[i].id === ant.path[j].id){
                console.log('JHGHJHGHJJKJJJJKKJKJKJKLJLKJKJLJKLJKLJLJLK');
                visited = true;
                break;
            }
        }
        ...
        const heuristic = ant.currentNode.score - Sensing.admissible_heuristic(cellFromUniqueID(ant.currentNode.id),cellFromUniqueID(graph[i].id))/* calculate heuristic information */;
