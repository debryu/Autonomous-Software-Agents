...
-----------------------------------------*/
const pop_size = 5000; // Population size
...
function createGraph(){
    ...
    for(let i=1; i<Env.PDmapDimension.x-1; i++){
        ...
        if(Env.PDMAP[Env.PDmapDimension.x-2][i] != 1 && Env.PDMAP[Env.PDmapDimension.x-2][i] != 1){
    ...
    for(let i=1; i<Env.PDmapDimension.y-1; i++){
        //Check if the cell is not a blocked cell
        if(Env.PDMAP[i][Env.PDmapDimension.y-2] != 1 && Env.PDMAP[i][Env.PDmapDimension.y-2] != 1){
