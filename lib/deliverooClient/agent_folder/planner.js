...
const iterations = 40; // Number of iterations
...
function evaluatePath(path){
    ...
    for(let i = 0; i < path.length - 1; i++){
        ...
        //TODO: add the time it takes to move to the next node
        let partial_time = Sensing.travelTime(cellFromUniqueID(path[i].id),cellFromUniqueID(path[i+1].id));
        time += partial_time;
...
function calculateProbabilities(ant, pheromoneTrails, graph) {
    ...
    for (let i = 0; i < graph.length; i++) {
        ...
        */
        const heuristic = ant.currentNode.score - Sensing.travelTime(cellFromUniqueID(ant.currentNode.id),cellFromUniqueID(graph[i].id));
...
function createGraph(){
    ...
    for(let i=1; i<Env.PDmapDimension.x-1; i++){
        ...
        if(Env.PDMAP[Env.PDmapDimension.x-2][i] != 1 && Env.PDMAP[Env.PDmapDimension.x-2][i] != 3 && Env.PDMAP[Env.PDmapDimension.x-2][i] == 2){
    ...
    for(let i=1; i<Env.PDmapDimension.x-1; i++){
        //Check if the cell is not a blocked cell
        if(Env.PDMAP[1][i] != 1 && Env.PDMAP[1][i] != 3 && Env.PDMAP[1][i] != 2){
    ...
    for(let i=1; i<Env.PDmapDimension.y-1; i++){
        //Check if the cell is not a blocked cell
        if(Env.PDMAP[i][1] != 1 && Env.PDMAP[i][1] != 3 && Env.PDMAP[i][1] != 2){
    ...
    for(let i=1; i<Env.PDmapDimension.y-1; i++){
        //Check if the cell is not a blocked cell
        if(Env.PDMAP[i][Env.PDmapDimension.y-2] != 1 && Env.PDMAP[i][Env.PDmapDimension.y-2] != 3  && Env.PDMAP[i][Env.PDmapDimension.y-2] != 2){
    ...
    return [nodes,parcelNodes,deliveryNodes];
}
