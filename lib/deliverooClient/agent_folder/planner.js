...
const std_min_reward_threshold = 5;
const std_delivery_cutoff = 1;
...
export function plan(min_rew_threshold = std_min_reward_threshold, delivery_cutoff = std_delivery_cutoff){
    ...
    */
    ...
    if(ant.pathScore >= min_rew_threshold){
        ...
        console.log('path',ant.path);
        console.log('The path score is too low with value ' + ant.pathScore + '<' + min_rew_threshold + '\nAborting...');
        return false;
    }
}
...
function evaluatePath(path){
    ...
    if(delivered){ //&& (Sensing.ME.score != Connection.agentSensing[1].score)){
        let fitness_score = score_mateo - time*(path.length-2);
        //console.log('time',time);
        //console.log('The ant delivered with score ',fitness_score, ' and time ' + time);
        if(fitness_score < 0)
            return 0;
        else 
...
function calculateProbabilities(ant, pheromoneTrails, graph) {
    ...
    for (let i = 0; i < graph.length; i++) {
        ...
        var pheromone = pheromoneTrails[ant.currentNode.idx][graph[i].idx];
        //console.log(ant)
        ...
        */
        var heuristic = ant.currentNode.score + graph[i].score - Sensing.travelTime(cellFromUniqueID(ant.currentNode.id),cellFromUniqueID(graph[i].id));
        //+ graph[i].score
...
export function cellFromUniqueID(ID){
    let x = ID%dim_x;
    let y = (ID-x)/dim_x;
    return {x: x, y: y};
}
...
function createGraph(delivery_cutoff){
    ...
    */
    ...
    // Need to re-numbered the id of the nodes
    let reoredered_index = 0;
    ...
    for(let i = 0; i < partialDeliveryNodesCount; i++){
        ...
        let node = deliveryNodes[i];
        let updatedNode = {x: node.x, y: node.y, idx: reoredered_index, delivery: node.delivery, score: 0, id: node.id};
        reoredered_index += 1;
        customNodes.push(updatedNode);
    }

    // Add all the parcel nodes to the customNodes array
    for(let i = 0; i < parcelNodes.length; i++){
        // Start counting again
        let node = parcelNodes[i];
        let updatedNode = {x: node.x, y: node.y, idx: reoredered_index, delivery: node.delivery, score: node.score, id: node.id};
        reoredered_index += 1;
        customNodes.push(updatedNode);
    }
    ...
    //console.log("Delivery nodes: ", deliveryNodes);
    //console.log("Parcel nodes: ", parcelNodes);
