...
export function astar(start,end){
    ...
    while (queue.size > 0){
        ...
        if(currentCell.x == end.x && currentCell.y == end.y){
            return recontructPath(cameFrom,currentCell);//Array.from(recontructPath(cameFrom,currentCell));
        }
...
export function convertPathToMoves(path){
    let actions = [];
    console.log("asdasdjknaskdjnsa", path)
