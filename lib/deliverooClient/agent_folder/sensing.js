...
export function SANEmhdF(){
    ...
    for(let x=1; x<Environment.PDmapDimension.x; x++){
        for(let y=1; y<Environment.PDmapDimension.y; y++){
            let startCell = {x: agentCell.x, y: agentCell.y};
            let endCell = {x: x, y: y};
            if(mhdMap[y][x] == 1 || x == 0 || y == 0 || x == Environment.PDmapDimension.x-1 || y == Environment.PDmapDimension.y-1){// padding or blocked cells
                mhdMap[y][x] = 'I';
            }
            else
