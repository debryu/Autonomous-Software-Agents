    ...
    function getDirection () {
        ...
        if (Parcel.parcelsArray.length){ // check if not empty
            ...
            if (closeParcels.length){
                console.log("SMART!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!:", closeParcels[0].x, closeParcels[0].y, Sensing.ME.x, Sensing.ME.y)
    ...
    while ( true ) {
        ...
        await new Promise( (success, reject) => socket.emit('move', getDirection(), async (status) =>  {
            if (status) {
                ...
                socket.on("you", ({id, name, x, y, score}) => {Sensing.ME.x =Math.round(x), Sensing.ME.y=Math.round(y);})
                console.log( 'moved', Sensing.ME.x, Sensing.ME.y, ' next move', direction_index);//, direction, 'next move', direction_index)
                
                await new Promise( res => setTimeout(res, 100) ); // wait 0.1 sec
                socket.emit( 'putdown' );
                if (Sensing.ME.carring)
                    Sensing.ME.carring = false; // dropped parcel. This value is updated in parcels.js -> findParcel()
