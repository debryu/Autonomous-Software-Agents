...
socket.on("you", function(data){
    ...
    if(Number.isInteger(data.x) && Number.isInteger(data.y)){
        ...
        console.log('agent pos:',Sensing.agentCell.x,Sensing.agentCell.y);
    }
...
async function ARE_YOU_WINNING_SON(){
    ...
    Environment.initializePDMAPcopy();

    Environment.initializeL1HeatMAP();
    Environment.initializeL2HeatMAP();
    Environment.initializeL3HeatMAP();
    Environment.initializeL4HeatMAP();
    Environment.initializeL5HeatMAP();
    Environment.printLMap(1);
    Environment.printLMap(5);

    //Compute the cell that is the best place to idle
    let idleCell = Sensing.findMaxMatrix(Environment.Layer5HeatMAP);
    console.log('Idle cell:', idleCell);
    ...
    while(true){
        ...
        let tEnd;
        console.log(Parcel.parcelsArray);
        ...
        console.log('current pos:', Sensing.agentCell);
        console.log('end goal',tEnd, ' or ', idleCell);
        ...
        console.log('planned moves:',planned_moves);
        ...
        if(planned_moves.length > 0){
            ...
            await new Promise( res => setTimeout(res, 10) ); // wait  10 sec
            await Action.pickup();
            await new Promise( res => setTimeout(res, 10) ); // wait  0.10 sec
        }
        else{
        ...
        console.log('ParcelsArray:',Parcel.parcelsArray);
        //Environment.restorePDMAP();
    }
}
