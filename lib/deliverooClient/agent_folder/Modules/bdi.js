...
export async function jones(){
    ...
    while(active){
        ...
        if(Sensing.canReachDeliveryCell()){ //Agent can deliver so go to delivery cell
            ...
            Pddl.initPDDL(farthestCell, false);
            pddlStuff = await Pddl.getPlan();
            await new Promise(res => setTimeout(res, 10));
            try{
                await Action.goTo(pddlStuff, true,1000000,3);
            }catch(err){
                var lastmove = err;
                console.log('Last attemted move:', err);
            }
            ...
            await new Promise(res => setTimeout(res, 10));
            try{
                await Action.goTo(pddlStuff, true, 1000000);
            }
            catch(err){
                console.log(err,'reached teammate');
                break;
            }

            Pddl.initPDDL(delivery[0]); //Try to go to the delivery
            pddlStuff = await Pddl.getPlan();
            await new Promise(res => setTimeout(res, 10));
            try{
                await Action.goTo(pddlStuff, true,1000000,3);
