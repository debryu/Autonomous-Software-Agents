...
export function connectedMap(seedBlock){
    ...
    while(queue.length > 0){
        ...
        for(let i = 0; i < neighbors.length; i++){
            //console.log(Env.PDMAP[neighbors[i].x][neighbors[i].y] != 1)
...
function countDeliveryCells(cells){
    ...
    for(let i = 0; i < cells.length; i++){
        let cell = cells[i];
        //console.log('cell',Env.PDMAP[cell.y][cell.x]);
...
export function farthestCell(objects, referencePoint) {
    function calculateDistance(point1, point2) {
      ...
      //console.log(point1,point2, dx,dy)
    ...
    for (let i = 1; i < objects.length; i++) {
      ...
      //console.log(distanceToReference, farthestDistance)
