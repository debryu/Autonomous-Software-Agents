...
export function travelTime(start,end){
    let timeFactor = Env.mapConfig.MOVEMENT_DURATION / 1000;
    let steps = astar(start,end).length;
...
export function astar(start,end){
    ...
    while (queue.size > 0){
        ...
        if(Env.PDMAP[nLEFT.y][nLEFT.x] != 1 && Env.PDMAP[nLEFT.y][nLEFT.x] != 3){
            neighbours.add(nLEFT);
            //console.log('LEFT', Env.PDMAP[nLEFT.y][nLEFT.x], 'from' , currentCell);
        }
            
        if(Env.PDMAP[nRIGHT.y][nRIGHT.x] != 1 && Env.PDMAP[nRIGHT.y][nRIGHT.x] != 3){
            neighbours.add(nRIGHT);
            //console.log('RIGHT', Env.PDMAP[nRIGHT.y][nRIGHT.x], 'from' , currentCell);
        }
            
        if(Env.PDMAP[nUP.y][nUP.x] != 1 && Env.PDMAP[nUP.y][nUP.x] != 3){
            neighbours.add(nUP);
            //console.log('UP', Env.PDMAP[nUP.y][nUP.x], 'from' , currentCell);
        }
            
        if(Env.PDMAP[nDOWN.y][nDOWN.x] != 1 && Env.PDMAP[nDOWN.y][nDOWN.x] != 3){
...
export function mhdFinder(){
    for(let x=1; x<Env.PDmapDimension.x-1; x++){
        for(let y=1; y<Env.PDmapDimension.y-1; y++){
            ...
            let endCell = {x: x, y: y};
            if(Env.PDMAP[y][x] == 1){// padding or blocked cells
                mhdMap[y][x] = 'I';
            }
            else
...
export function closestDelivery(cell){
    ...
    //TODO: 
    // - Check if the cell is not a blocked cell by another agent
