...
export var PDMAPbkp = Array.from({ length: PDmapDimension.y }, () => Array(PDmapDimension.x).fill(1));
/*-----------------------------------------------------
    FUNCTIONS
-------------------------------------------------------*/
...
export function initializePDMAP(){
    ...
    */   
    //Create a copy of it to revert to original values
    ...
    for (let i=PDmapDimension.y-1; i>=0; i--){
        ...
        for (let j=0; j < PDmapDimension.x; j++){
            ...
            if(PDMAPbkp[i][j] == 2)
                //red
                color = 31
            else if (PDMAPbkp[i][j] == 0)
                //green
                color = 32
            else
                //gray
                color = 30
            let symbol = PDMAPbkp[i][j].toString();
            //ADJUSTED FOR PADDED MAP
            if(i == Sensing.agentCell.y && j == Sensing.agentCell.x)
                symbol = "X";
            scanLine = scanLine + " " + Utils.colorizeString(symbol, color);
