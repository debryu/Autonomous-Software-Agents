...
export function findParcel(pp){
    if(pp != undefined){
      if(pp.id != null){ //just to be sure
        ...
        if(pp.carriedBy == Sensing.ME.id){
          Sensing.ME.carring = true; // Track if agent is carring something or not
          Sensing.mhdFinder(); //Calculate manhatan distances
          console.log("CARRINGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGG ", Sensing.bordersArray);
