...
export default class PddlAction {
    ...
    * iterateOnEffects () {
    ...
    tokenize ( preconditionOrEffect ) {
        ...
        if (preconditionOrEffect.length ==3){
            preconditionOrEffect[2] = '?'+preconditionOrEffect[2];
            let merged = preconditionOrEffect.slice(-2).join(' ');
            preconditionOrEffect.splice(-2, 2, merged);
        }
        //console.log(preconditionOrEffect)
...
${padding}:precondition (${this.operation}
